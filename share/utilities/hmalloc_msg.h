...
typedef struct __attribute__((packed)) {
    uint64_t addr;
    uint32_t hid;
} hmalloc_update_hid_msg;

typedef struct __attribute__((packed)) {
    uint32_t hid;
    char     label[32];
} hmalloc_label_hid_msg;
...
typedef struct __attribute__((packed)) {
    ...
    union {
        ...
        hmalloc_update_hid_msg updt;
        hmalloc_label_hid_msg  labl;
...
static uint32_t _hmsg_size(hmalloc_msg *msg) {
    ...
    switch (msg->header.msg_type) {
        ...
        case HMSG_ALLO:
            ...
            break;
        case HMSG_UPDT:
            size += sizeof(hmalloc_update_hid_msg);
            break;
        case HMSG_LABL:
            size += sizeof(hmalloc_label_hid_msg);
            break;
        case HMSG_FREE:
...
int hmsg_start_receiver(hmalloc_msg_receiver *receiver) {
    ...
    old_mode = umask(0);
    if ((receiver->qd = mq_open(HMSG_MQ_NAME, O_RDONLY | O_CREAT | O_EXCL, 0666, &attr)) == -1) {
        err   = -errno;
        errno = 0;
        goto out;
    }
