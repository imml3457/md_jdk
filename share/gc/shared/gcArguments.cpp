...
void GCArguments::initialize() {
  ...
  if ( (!FLAG_IS_DEFAULT(AllocateOldGenAt)) || (!(FLAG_IS_DEFAULT(AllocateOldGenOnNVRAM))) ) {
  ...
  if (!FLAG_IS_DEFAULT(AllocateOldGenOnNVRAM)) {
...
bool GCArguments::check_args_consistency() {
  ...
  if (!FLAG_IS_DEFAULT(AllocateHeapAt) && !FLAG_IS_DEFAULT(AllocateOldGenOnNVRAM)) {
    jio_fprintf(defaultStream::error_stream(),
      "AllocateHeapAt and AllocateOldGenOnNVRAM cannot be used together.\n");
    status = false;
  }
  if (!FLAG_IS_DEFAULT(AllocateOldGenAt) && !FLAG_IS_DEFAULT(AllocateOldGenOnNVRAM)) {
    jio_fprintf(defaultStream::error_stream(),
      "AllocateOldGenAt and AllocateOldGenOnNVRAM cannot be used together.\n");
    status = false;
  }
