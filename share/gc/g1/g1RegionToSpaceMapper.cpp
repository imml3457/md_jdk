...
class G1RegionsLargerThanCommitSizeMapper : public G1RegionToSpaceMapper {
 ...
 public:
  ...
  virtual void commit_regions(uint start_idx, size_t num_regions, WorkGang* pretouch_gang) {
    ...
    //tty->print_cr("__commit: start: %u num: %lu type: %u",
    //                 start_idx, num_regions, _memory_type);
...
bool G1RegionToHeteroSpaceMapper::initialize() {
  ...
  ReservedSpace rs_nvram = _rs.first_part(MaxHeapSize);
  assert(rs_nvram.base() == _rs.base(), "We should get the same base address");

  // Second half of reserved memory is mapped to dram.
  ReservedSpace rs_dram = _rs.last_part(MaxHeapSize);

  assert(rs_dram.size() == rs_nvram.size() && rs_nvram.size() == MaxHeapSize, "They all should be same");
  ...
  if (AllocateOldGenOnNVRAM) {
    // Reserve nvram memory
    char* base = os::attempt_reserve_memory_at(rs_nvram.size(), rs_nvram.base());
    if (base != rs_nvram.base()) {
      if (base != NULL) {
        os::release_memory(base, rs_nvram.size());
      }
      log_error(gc, init)("Error in re-mapping memory on nvram during G1 heterogenous memory initialization");
      return false;
    }
  } else {
    // We reserve and commit this entire space to NV-DIMM.
    if (!map_nvdimm_space(rs_nvram)) {
  ...
  _start_index_of_dram = (uint)(rs_nvram.size() / _region_granularity);

  if (AllocateOldGenOnNVRAM) {
    if (_region_granularity >= (_page_size * _commit_factor)) {
      _nvram_mapper = new G1RegionsLargerThanCommitSizeMapper(rs_nvram, rs_nvram.size(), _page_size, _region_granularity, _commit_factor, _type);
    } else {
      _nvram_mapper = new G1RegionsSmallerThanCommitSizeMapper(rs_nvram, rs_nvram.size(), _page_size, _region_granularity, _commit_factor, _type);
    }
  }
...
void G1RegionToHeteroSpaceMapper::commit_regions(uint start_idx, size_t num_regions, WorkGang* pretouch_gang) {
  ...
  uint num_nvram = (uint)num_regions - num_dram;

  //tty->print_cr("commit:   start: %u end: %u num_dram: %u num_nvram: %u",
  //               start_idx, end_idx, num_dram, num_nvram);
  if (num_nvram > 0) {
    if (AllocateOldGenOnNVRAM) {
      guarantee(start_idx < _start_index_of_dram, "must be");
      _nvram_mapper->commit_regions(start_idx, num_nvram, pretouch_gang);
    }
    _num_committed_nvram += num_nvram;
  }
...
void G1RegionToHeteroSpaceMapper::uncommit_regions(uint start_idx, size_t num_regions) {
  ...
  if (num_nvram > 0) {
    if (AllocateOldGenOnNVRAM) {
      guarantee(start_idx < _start_index_of_dram, "must be");
      _nvram_mapper->uncommit_regions(start_idx, num_nvram);
    }
    _num_committed_nvram -= num_nvram;
  }
...
uint G1RegionToHeteroSpaceMapper::num_committed_nvram() const {
  return _num_committed_nvram;
}
...
G1RegionToSpaceMapper* G1RegionToSpaceMapper::create_heap_mapper(ReservedSpace rs,
                                                                 ...
                                                                 size_t commit_factor,
                                                                 MemoryType type) {
  if ( AllocateOldGenOnNVRAM || (AllocateOldGenAt != NULL) ) {
