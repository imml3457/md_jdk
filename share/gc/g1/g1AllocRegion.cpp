...
HeapWord* G1AllocRegion::new_alloc_region_and_allocate(size_t word_size,
  ...
  if (new_alloc_region != NULL) {
   ...
   //printf("bottom addr " PTR_FORMAT " top addr " PTR_FORMAT "\n", p2i(new_alloc_region->bottom()), p2i(new_alloc_region->end()));
   //printf("calculated difference " PTR_FORMAT " and used space %d\n", p2i(new_alloc_region->top()) - p2i(new_alloc_region->bottom()), new_alloc_region->is_empty());
    //printf("block size for region %zx\n", new_alloc_region->block_size(new_alloc_region->end()));
    //printf("capacity %zx and free %zx\n", new_alloc_region->capacity(), new_alloc_region->free()); 
   // new_alloc_region->print();   
    //new_alloc_region->verify();
