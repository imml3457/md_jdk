...
class G1NUMA: public CHeapObj<mtGC> {
  ...
  int* _node_ids;
  int* _dram_idx_to_node_idx;
  int* _nvram_idx_to_node_idx;

  // Total number of node ids.
  uint _num_active_node_ids;
  uint _num_active_dram_node_ids;
  uint _num_active_nvram_node_ids;
...
public:
  ...
  void set_region_info(size_t region_size, size_t page_size, uint start_index_of_dram);
