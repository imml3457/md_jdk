...
/*class messageAll{
    ...
    void sendmsg(uint64_t addr, uint64_t size, uint32_t hid, int m){
        if(m == 0){
            hmalloc_msg msg;
            struct timespec ts;
    ...
    void updatemsg(uint64_t addr, uint32_t hid){
        hmalloc_msg msg;
...
HeapRegion* G1CollectedHeap::new_heap_region(uint hrs_index,
                                             MemRegion mr) {
  HeapRegion* hr = new HeapRegion(hrs_index, bot(), mr);
...
HeapRegion* G1CollectedHeap::new_region(size_t word_size,
  ...
  uint ty = 0;
   ...
   //res->print();
  //printf("number of used regions %u\n", num_used_regions());
  msg.sendmsg(p2i(res->bottom()), res->capacity(), ty, 0);
  msg.updatemsg(p2i(res->bottom()), ty);
...
bool G1CollectedHeap::alloc_archive_regions(MemRegion* ranges,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    msg.sendmsg(p2i(curr_region->bottom()), curr_region->capacity(), 5, 0);
...
/*   msg.sendmsg(0x000000011f000000, 0x6E1000000, 0, 0); */
  ...
  _numa->set_region_info(HeapRegion::GrainBytes, page_size,
    AllocateOldGenOnNVRAM ? ((G1RegionToHeteroSpaceMapper*)
                              heap_storage)->start_index_of_dram() : 0);
...
void G1CollectedHeap::print_md(){
  if(_hrm == NULL){
	return;
  }
  
  printf("printing regions %d\n", this->num_regions());
...
void G1CollectedHeap::free_region(HeapRegion* hr, FreeRegionList* free_list) {
  ...
  msg.sendmsg(p2i(hr->bottom()), 0, 0, 1);
  if (G1VerifyBitmaps) {
...
void G1CollectedHeap::retire_mutator_alloc_region(HeapRegion* alloc_region,
  ...
  //msg.sendmsg(p2i(alloc_region->bottom()), 0, 0, 1);
...
HeapRegion* G1CollectedHeap::new_gc_alloc_region(size_t word_size, G1HeapRegionAttr dest, uint node_index) {
  ...
  if (new_alloc_region != NULL) {
   ...
   // msg.sendmsg(p2i(new_alloc_region->bottom()), new_alloc_region->capacity(), ty, 0);
/*     msg.updatemsg(p2i(new_alloc_region->bottom()), ty); */
    return new_alloc_region;
  }
