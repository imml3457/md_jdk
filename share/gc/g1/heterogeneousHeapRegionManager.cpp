...
uint HeterogeneousHeapRegionManager::total_regions_committed() const {
  return num_committed_dram() + num_committed_nvram();
}
...
uint HeterogeneousHeapRegionManager::num_committed_nvram() const {
  // See comment for num_committed_dram()
  return static_cast<G1RegionToHeteroSpaceMapper*>(_heap_mapper)->num_committed_nvram();
}
