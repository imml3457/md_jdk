...
void G1NUMA::initialize(bool use_numa) {
  ...
  /* MRJ -- not portable at all -- need to rewrite to be general for NUMA
   * machines with DRAM and NVRAM
   */
  _dram_idx_to_node_idx = NEW_C_HEAP_ARRAY(int, 1, mtGC);

  if (G1DRAMIsReallyNVRAM) {
    _dram_idx_to_node_idx[0] = 1;
  } else {
    _dram_idx_to_node_idx[0] = 0;
  }

  _num_active_dram_node_ids = 1;

  _nvram_idx_to_node_idx = NEW_C_HEAP_ARRAY(int, 1, mtGC);

  if (G1NVRAMIsReallyDRAM) {
    _nvram_idx_to_node_idx[0] = 0;
  } else {
    _nvram_idx_to_node_idx[0] = 1;
  }

  _num_active_nvram_node_ids = 1;
...
void G1NUMA::set_region_info(size_t region_size, size_t page_size,
  uint start_index_of_dram) {
  _region_size = region_size;
  _page_size = page_size;
  _start_index_of_dram = start_index_of_dram;
}
...
uint G1NUMA::preferred_node_index_for_index(uint region_index) const {
  uint active_nodes, index;

  if (AllocateOldGenOnNVRAM) {
    if (region_index < _start_index_of_dram) {
      active_nodes = _num_active_nvram_node_ids;
    } else {
      active_nodes = _num_active_dram_node_ids;
    }
  } else {
    active_nodes = _num_active_node_ids;
  }

  if (region_size() >= page_size()) {
    // Simple case, pages are smaller than the region so we
    // can just alternate over the nodes.
    index = region_index % active_nodes;
  } else {
    ...
    index = (region_index / regions_per_page) % active_nodes;
  }

  if (AllocateOldGenOnNVRAM) {
    if (region_index < _start_index_of_dram) {
      return _nvram_idx_to_node_idx[index];
    } else {
      return _dram_idx_to_node_idx[index];
    }
  }
...
//      * NUMA node #:  |----#0----||----#1----||----#2----||----#3----||----#0----||----#1----||----#2----||----#3----|
void G1NUMA::request_memory_on_node(void* aligned_address, size_t size_in_bytes,
  uint region_index) {
  if (!is_enabled()) {
    return;
  }
  ...
  //tty->print_cr("Request memory [" PTR_FORMAT ", " PTR_FORMAT ") to be NUMA id (%d)",
