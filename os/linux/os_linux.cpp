...
static char* anon_mmap(char* requested_addr, size_t bytes, bool fixed) {
  ...
  addr = (char*)::mmap(requested_addr, bytes, PROT_NONE,
                       flags, -1, 0);
/*   if((uint64_t)addr != 0x11f000000 || (uint64_t)addr != 0x800000000){                      */
/*         msg.sendmsg((uint64_t)addr, bytes, 11, 0); */
/*         printf("addr %lx\n size: %lu\n", (uint64_t)addr, bytes/(1024*1024*1024)); */
/*     } */
/*   printf("address space for anon-mmap %x */
...
/*   if((uint64_t)start != 0x11f000000 || (uint64_t)start != 0x800000000){                      */
/*     msg.sendmsg((uint64_t)start, bytes, 10, 0); */
/*     printf("addr %lx\n size: %lu\n", (uint64_t)start, bytes/(1024*1024*1024)); */
/* } */
  if (start == MAP_FAILED) {
    start = NULL;
  } else {
